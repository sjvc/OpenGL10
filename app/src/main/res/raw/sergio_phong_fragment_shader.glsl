...
void main()
{
	...
	float specular = pow(max(dot(V, R), 0.0), 150.0);		// Exponente de Phong

	gl_FragColor = u_Color*ambient+attenuation*(u_Color*diffuse + specularColor*specular);
	...
	specular = pow(max(dot(V, R), 0.0), 150.0);				// Exponente de Phong
